...
type
  TIFKProgrammierungForm = class(TForm)
    ...
    function FailSaveEnable():boolean;
...
//var Anwendungsverzeichnis:string;
...
begin
     ...
     PCI_TimerWait(Cardauswahl, UserSaveEnableTimeMS, 1, ErrStatus);
     if(ErrStatus = 0) then result:= true
     else result:= false;
end;

function TIFKProgrammierungForm.FailSaveEnable():boolean;

var ErrStatus:_DWORD;

begin
     ErrStatus:=0;
     modulbus_adr_wr(mod_cstat_reload_failsave, ifk_comm_status_wr, MBKAdress, 1,ErrStatus);
...
//     if(ProgramTestVer = true) then begin
       //================================================================
       ...
       status:= PCI_IfkWrite(Cardauswahl, IFKAdressMaster, FCWR_DirectTreiber, ProgSetMasterOut, ErrStatus);
       if(status<> StatusOK) then ErrorOut.Add('Err.SetMasterDriverDirection: $'+IntToHex(status,4));

       //Slave muss seine adresse ueber den master bekommen
       //dazu wird d0-15 die adresse vom slvae (2) angelegt
       status:= PCI_IfkWrite(Cardauswahl, IFKAdressMaster, WrVgl1, IFKAdressSlave, ErrStatus);
       if(status<> StatusOK) then ErrorOut.Add('Err.SetSlaveAdresse: $'+IntToHex(status,4));

       //mastertreiber anablen
       status:= PCI_IfkWrite(Cardauswahl, IFKAdressMaster, FCWR_EnabelTreiber, EnableTreiber, ErrStatus);
       if(status<> StatusOK) then ErrorOut.Add('Err.MasterDriverEnabel: $'+IntToHex(status,4));
...
//     end;
     ...
     if(ProgramTestVer = true) then statebool:= LoadFlashFile(DateiTestFlash)
     else  statebool:= LoadFlashFile(DateiTestFinal);

     //wegen der entprellung der interfacekartenadresse erst mal einen kaffee
     PCI_TimerWait(Cardauswahl, 100, 1, ErrStatus);

      //Flash programmieren
     if (statebool = false) then begin
        Application.MessageBox('Test Konfigurationsfile konnte nicht geladen werden !','Shit',16);
        ErrorFound:= true
     end else statebool:=UserFlashprog(ErrorOut);
     if(statebool = false) then ErrorFound:= true;

     //noch einen Kaffee
     PCI_TimerWait(Cardauswahl, 1000, 1, ErrStatus);

     //Programmiertes flash ueberprüfen
     if(ErrorFound = false) then statebool:= UserFlashVerify(ErrorOut);
     if(statebool = false) then ErrorFound:= true;
     ...
     if(ErrorFound = false) then begin
       ...
       Application.ProcessMessages();
       statebool:= UserSaveEnable();
       BitteWartenForm.Close;
       Application.ProcessMessages();
     end;

    //noch einen kaffee hinterher
    // PCI_TimerWait(Cardauswahl, 5000, 1, ErrStatus);


     if(ProgramTestVer = false) then begin
